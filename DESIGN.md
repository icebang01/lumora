...
- **浮动窗 accent 令牌**（桌面歌词 `desktop-lyrics.html`、画中画 `pip.html`）：
  ...
  - 画中画 `pip.html` 不使用 `--dl-accent`，自带局部 `rgba()` 控制条（见下条玻璃背景）。
  - 暗色玻璃背景 `rgba(14,16,28,.86)` / `rgba(18,20,32,.6)`（桌面歌词为透明叠窗、无玻璃）与投屏面板 `rgba(12,13,18,.94)` 近似但未统一 —— 属 `pip.html` 另案，非 `--dl-accent` 漂移。
  - 大量 `rgba(255,255,255,α)` 文本色属浮动窗叠加桌面场景的合理选择，非严格令牌化；当前判定为**有意设计**（浮动窗叠在桌面之上），保留不强制改引 `--text-1/2/3`。
  - **结论（2026-08-14）**：`--dl-accent` 漂移已消除（回退对齐品牌 + 死令牌删除）；浮动窗允许独立文本调色板为有意设计，不再列为待修漂移。玻璃统一留待 `pip.html` 单独处理。
