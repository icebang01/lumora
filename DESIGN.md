...
| 许可证 | `panels/licenses.js` | DIALOG | ✅ | `LICENSES_MOCK.html`（独立窗口 data-light-surface · mpv/FFmpeg/Electron/ws 许可要点） |
| 画中画 | `pip.html` `pip-preload.js` | FLOATING | ✅ | `PIP_MOCK.html`（透明浮窗 · 顶栏控制 + 保持宽高比缩放，见 A.3 已闭环（有意设计）） |
| 桌面歌词 | `desktop-lyrics.html` | FLOATING | ✅ | `DESKTOP_LYRICS_MOCK.html`（逐字着色 + 字号/字体/锁定，见 A.3 已闭环（有意设计）） |
...
- **浮动窗 accent 令牌**（桌面歌词 `desktop-lyrics.html`、画中画 `pip.html`）：
  - 桌面歌词**无** `:root` 声明 `--dl-accent`；仅 `var(--dl-accent, #7c8cff)` 回退（原回退 `#4d8dff` 已于 2026-08-14 对齐品牌 `#7c8cff`）。运行时主进程每帧 `setProperty('--dl-accent', data.accent)` 注入真实品牌 accent，**正常路径无可见漂移**，回退仅作用于「未转发 accent」的极端边界。
  - 曾存在的 `--dl-accent-a/b/c` 三件套（cyan/purple/pink）**全文零引用**，已于 2026-08-14 删除（死令牌清理，无需保留）。
  - 画中画 `pip.html` 不使用 `--dl-accent`；其唯一玻璃是 `.pip-bar` 控制条 `rgba(20,20,25,.78)` + `blur(14px)`，且 `opacity:0` 默认隐藏、悬停才显现 —— 这是**叠在视频之上的 28px 悬浮控制条**，半透明是有意让它透出底下视频，并非对话框。
  - 暗玻璃核查（2026-08-14 再审）：A.3 原笔记称 `rgba(14,16,28,.86)` / `rgba(18,20,32,.6)` 与投屏 `rgba(12,13,18,.94)` 未统一 —— **经全仓库 grep，那两个 `.86/.6` 值根本不存在**（与 `--dl-accent` 误记同源）。真实玻璃面板（`style.css` 对话框/面板）清一色 `rgba(12,13,18,.9x)`（.93/.94/.96），已自洽。唯一离群即 pip.html 的 `.pip-bar` `.78`。
  - **玻璃统一结论（2026-08-14）**：`pip.html` 控制条 `.78` 是视频叠层的半透明悬浮条，强行统一到对话框 `.94` 会遮住视频、反而更差；属**有意设计，非漂移**，不改动源码。A.3「玻璃统一」项据此闭环为无需修复。
  - 大量 `rgba(255,255,255,α)` 文本色属浮动窗叠加桌面场景的合理选择，非严格令牌化；当前判定为**有意设计**（浮动窗叠在桌面之上），保留不强制改引 `--text-1/2/3`。
  - **结论（2026-08-14 终审）**：`--dl-accent` 漂移已消除（回退对齐品牌 + 死令牌删除）；`--accent-2` 未定义已修复（→ `--accent-pink`，提交 `3374dae`）；浮动窗独立文本调色板、`pip.html` 控制条半透明玻璃均判定为**有意设计**；A.3 原「玻璃统一」项经全仓库 grep 证实为误报（`.86/.6` 值不存在，真实面板已自洽 `rgba(12,13,18,.9x)`）。**A.3 已知漂移全部闭环，无待修源码改动。**
